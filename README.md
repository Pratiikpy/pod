...
- **SoDEX testnet trading needs the wallet whitelisted.** Until the SoDEX team whitelists the demo wallet for trading, `/trade` will reach the order step and come back with the API gate error. The confirmation flow, signing, and submission all run; the order just gets rejected at the venue.
